# ClubCoin — the URL-callback round trip
...
1. The user is on ClubCoin and taps **Continue with Passport**.
2. ClubCoin mints a state token, stores it in `sessionStorage`, and navigates to
...
3. Passport carries those parameters through the entire onboarding — create or
   discover a passkey, open the wallet, optionally claim a name — and shows the
   consent sheet once a session is open.
4. On **Share**, Passport redirects back to ClubCoin with the reply in the URL
   **fragment**, signed with the wallet's unshielded key. On **Don't share** it
   redirects with `#passportError=denied`.
5. ClubCoin verifies the reply before believing any of it, then renders the
   member.
...
therefore does not appear in ClubCoin's access logs, in its reverse proxy's
...
| `audience` equals ClubCoin's own origin | a reply harvested from a different app being replayed here |
| `state` echoes the token ClubCoin sent | a reply injected into a session ClubCoin did not start |
...
If the Passport session had no key that could sign — a Dynamic-hosted session
with no local wallet — the reply arrives with `scheme: "none"` and no
signature. ClubCoin refuses it by default. `VITE_ACCEPT_UNSIGNED=1` accepts it
and says so on screen; it is never presented as verified.
...
audience binding. Point ClubCoin at a deployed Passport with
