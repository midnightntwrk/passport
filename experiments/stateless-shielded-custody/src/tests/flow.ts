...
export interface StatelessSetup {
  ctx: TestContext;
  faucet: FaucetHandle;
  custody: StatelessCustody;
  encKeys: EncKeyPair;
}

export async function standardSetup(): Promise<StatelessSetup> {
  step('setup: wallet, faucet, custody contract');
  const ctx = await setupWallet();
  const faucet = await deployFaucet(ctx.walletCtx);
  console.log(`  faucet  @ ${faucet.address}`);
  const encKeys = generateEncKeyPair();
  const custody = await deployCustody(ctx, encKeys);
  console.log(`  custody @ ${custody.address}`);
  return { ctx, faucet, custody, encKeys };
}
