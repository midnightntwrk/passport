...
export function compiledStatelessContract() {
  return CompiledContract.make('stateless', Contract).pipe(
...
export async function deployCustody(ctx: TestContext, encKeys: EncKeyPair): Promise<StatelessCustody> {
  const custody = await StatelessCustody.deploy(ctx.providers, compiledStatelessContract(), encKeys);
  fs.writeFileSync(
    DEPLOYMENT_FILE,
    JSON.stringify(
      { contractAddress: custody.address, deployedAt: new Date().toISOString() },
      null,
      2,
    ),
  );
...
export async function connectCustody(
  ...
  return StatelessCustody.connect(ctx.providers, compiledStatelessContract(), address, initialState);
}
