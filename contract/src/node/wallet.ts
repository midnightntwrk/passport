...
// UserAddress argument expects them. This is the bech32-decoded ADDRESS
// (the hash the ledger indexes UTXOs by), NOT the raw signing public key —
// tokens sent to the public-key bytes land at an address nobody owns.
export function userAddressBytes(walletCtx: WalletContext): Uint8Array {
  ...
  const decoded: any = (UnshieldedAddress as any).codec.decode(getNetworkId() as any, bech32 as any);
  const data: Uint8Array = decoded?.data ?? decoded?.bytes;
  if (!(data instanceof Uint8Array) || data.length !== 32) {
    throw new Error('failed to decode the unshielded address into 32 bytes');
  }
